  ...
  */
  ...
  public:
    ...
    */
    ...
    std::string syntax;  /** @brief 命令行语法（可自动生成，如无必要可不比改）  */
  ...
  private:
    ...
    /*members*/
    std::map< std::string, int > optionGroupIds;  /** @brief 工具名称，直接从argv中解析得到  */
    std::vector<OptionGroup> groups;  /** @brief 参数组  */
    std::vector<std::string> xorGroups;  /** @brief 互斥参数组  */
    std::vector<std::string> unknownOptions;  /** @brief 无标签参数组  */
    std::map<int, std::string> unlabeledPos; /** @brief 无标签参数组名称位置对应表  */
  };
}
/* ################################################################### */
